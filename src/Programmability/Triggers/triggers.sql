...
-- ON_HANDLE_CREATE_COMPANY_WEBHOOK
create or replace function public.handle_create_company_webhook()
    returns trigger
    language plpgsql
    security definer set search_path = extensions, public
as
...
begin
    ...
    from http((
               'POST',
               'https://kickscan.vercel.app/api/supabase/company_webhook_created',
               ARRAY [http_header('Authorization', 'Basic bnV1cmNvZGVzOnZCKjBnYmpVQjdLejRFN14mZHhITV5acg==')],
               'application/json',
               jsonb_build_object('record',
                                  jsonb_build_object('company_id', new.company_id))
...
-- ON_HANDLE_UPDATE_COMPANY_WEBHOOK
create or replace function public.handle_update_company_webhook()
    returns trigger
    language plpgsql
    security definer set search_path = extensions, public
as
...
begin
    ...
    from http((
               'POST',
               'https://kickscan.vercel.app/api/supabase/company_webhook_updated',
               ARRAY [http_header('Authorization', 'Basic bnV1cmNvZGVzOnZCKjBnYmpVQjdLejRFN14mZHhITV5acg==')],
               'application/json',
               jsonb_build_object('record',
                                  jsonb_build_object('company_id', new.company_id))
...
-- ON_HANDLE_DELETE_COMPANY_WEBHOOK
create or replace function public.handle_delete_company_webhook()
    returns trigger
    language plpgsql
    security definer set search_path = extensions, public
as
...
begin
    ...
    from http((
               'POST',
               'https://kickscan.vercel.app/api/supabase/company_webhook_deleted',
               ARRAY [http_header('Authorization', 'Basic bnV1cmNvZGVzOnZCKjBnYmpVQjdLejRFN14mZHhITV5acg==')],
               'application/json',
               jsonb_build_object('record',
                                  jsonb_build_object('company_id', new.company_id))
...
-- TRIGGER
drop trigger if exists on_company_webhook_created on public.company_webhook;
create trigger on_company_webhook_created
    after insert
    on public.company_webhook
    for each row
execute procedure public.handle_create_company_webhook();

-- TRIGGER
drop trigger if exists on_company_webhook_updated on public.company_webhook;
create trigger on_company_webhook_updated
    after update
    on public.company_webhook
    for each row
execute procedure public.handle_update_company_webhook();

-- TRIGGER
drop trigger if exists on_company_webhook_deleted on public.company_webhook;
create trigger on_company_webhook_deleted
    after delete
    on public.company_webhook
    for each row
execute procedure public.handle_delete_company_webhook();